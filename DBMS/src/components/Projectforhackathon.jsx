...
const initialProjectState = {
    ...
    project_genre: '',
    git_repo: '',
    demo_link: '', // <-- Added YouTube link state
    motivation: '',
    overview: '',
    features: []
};
...
export default function Addproject() {
    ...
    const handleFormSubmit = async (e) => {
        ...
        const dataToSubmit = { ...projectData, team_id };

        try {
            await axios.post('http://localhost:4000/api/project/team-project', dataToSubmit, {
        ...
        <div className="bg-[#E9F0FF] min-h-screen font-sans">
            <main className="container mx-auto px-4 py-12">
                <form onSubmit={handleFormSubmit} className="max-w-5xl mx-auto flex flex-col gap-8">
                    ...
                    <section className="bg-white p-8 rounded-xl shadow-lg">
                        <h2 className="text-3xl font-bold text-gray-800 mb-6 pb-3 border-b">Demo Link</h2>
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            <input 
                                ...
                                placeholder="Enter your YouTube video URL (optional)" 
                                className={`${inputStyles} flex-grow`} 
                            />
                        </div>
                    </section>
                    {/* --- END: NEW YOUTUBE LINK SECTION --- */}
