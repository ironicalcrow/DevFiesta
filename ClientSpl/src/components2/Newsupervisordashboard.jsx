...
const WeeklyMarkingForm = ({ teams, onSaveMark, spldata }) => {
    ...
    const handleMemberSubmit = async (teamId, member) => {
        ...
        console.log(teamId, member.username, memberData.mark, memberData.comment)
        ...
        const form = {
            ...
            pbl_criteria_ids: [5],
            presentation_type: 3,
            marks: memberData.mark,
            comments: memberData.comment || ' '
        }
        console.log(form)
        const token = localStorage.getItem('token')
        const response = await axios.post('http://localhost:4000/api/pbl/judge/marking', form, {
                            ...
                            return (
                                <div key={member.username} className="bg-slate-50 border border-slate-200 p-4 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 font-semibold text-white bg-[#4060C1] rounded-lg shadow-sm hover:bg-sky-600 transition-colors w-full md:w-auto"
                                    >
                                        Submit
                                    </button>
                                </div>
...
export default function App() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-sky-50 to-white font-sans">
            <header className="bg-white/90 backdrop-blur-lg border-b border-slate-200 shadow-sm  top-0 z-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex gap-4 mb-8">
                    <button onClick={() => setActiveTab("teams")} className={`px-6 py-3 text-sm font-bold rounded-lg transition-all duration-200 ${activeTab === 'teams' ? 'bg-[#4060C1] text-white shadow-md' : 'bg-white text-slate-600 hover:bg-sky-100/50'}`}>
                        My Teams
                    </button>
                    <button onClick={() => setActiveTab("marking")} className={`px-6 py-3 text-sm font-bold rounded-lg transition-all duration-200 ${activeTab === 'marking' ? 'bg-[#4060C1] text-white shadow-md' : 'bg-white text-slate-600 hover:bg-sky-100/50'}`}>
                        Give Marks
                    </button>
                </div>
                <div>
                    {activeTab === "teams" ? (
                        ...
                        <WeeklyMarkingForm teams={teams} spldata={spldata} onSaveMark={handleSaveSingleMark} />
                    )}
                </div>
            </main>
        </div>
