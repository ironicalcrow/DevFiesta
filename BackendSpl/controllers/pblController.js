 ...
 static async getSupervisorUsername(req, res) {
        try {
            ...
            if (supervisor.length === 0) {
                return ResponseHandler.notFound(res, `Supervisor with ID ${supervisor_id} not found`);
            }

            return ResponseHandler.success(res, supervisor[0], `Supervisor username fetched successfully`);
        } catch (error) {
            console.error("Error fetching supervisor username:", error);
            return ResponseHandler.error(res, "Failed to fetch supervisor username", 500, error.message);
        }
    }
...
static async get_team_judge_marks(req, res) {
  try {
    ...
    if (!Number.isInteger(pblIdNum) || !Number.isInteger(presTypeNum)) {
      return ResponseHandler.badRequest(res, "pbl_id and presentation_type must be integers");
    }
    let teamIdNum = undefined;
    if (team_id !== undefined) {
      teamIdNum = Number(team_id);
      if (!Number.isInteger(teamIdNum)) {
        return ResponseHandler.badRequest(res, "team_id must be an integer when provided");
      }
    }

    const marks = await PBL.get_team_judge_marks(username, pblIdNum, presTypeNum, teamIdNum);

    if (!marks || marks.length === 0) {
      return ResponseHandler.notFound(res, "No marks found for this judge/team");
    }
    ...
    return ResponseHandler.error(res, "Failed to retrieve team judge marks", 500, error.message);
  }
}
