...
const leader_board = await pool.execute(`
  SELECT 
      t.team_id,
      t.team_name,
      COALESCE(marks_table.total_marks, 0) AS total_marks,
      RANK() OVER (ORDER BY COALESCE(marks_table.total_marks, 0) DESC) AS team_rank
  FROM team_participants tp
  JOIN teams t 
      ON tp.team_id = t.team_id
  LEFT JOIN (
      SELECT team_id, hackathon_id, SUM(marks) AS total_marks
      FROM marking
      GROUP BY team_id, hackathon_id
  ) AS marks_table
      ON marks_table.team_id = tp.team_id
     AND marks_table.hackathon_id = tp.hackathon_id
  WHERE tp.hackathon_id = ?
  GROUP BY t.team_id, t.team_name, marks_table.total_marks
