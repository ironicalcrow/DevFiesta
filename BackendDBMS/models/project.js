...
class Project {
  ...
  static async createProject_for_team(projectData, team_id) {
    ...
    const [projectResult] = await pool.execute(
      `INSERT INTO projects (
        project_name, git_repo, overview, motivation, features, project_genre,demo_link,creation_date
      ) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
      [project_name, git_repo, overview, motivation, features, project_genre,demo_link]
    );
